...
def visualize_boundary_linear(X, y, model, title=None):
    """
    X, y为loadData函数返回值
...
def main():
    X, y = load_data('data/task1_linear.mat')
    plot_data(X, y)
    model = svm_train_SMO(X, y, C=1, kernel_function='linear', max_iter=20)
    visualize_boundary_linear(X, y, model)

    X, y = load_data('data/task1_gaussian.mat')
    plot_data(X, y)
    model = svm_train_SMO(X, y, C=1, kernelFunction='gaussian', K_matrix=gaussian_kernel(X, sigma=0.1))
    visualize_boundary_gaussian(X, y, model, sigma=0.1)
